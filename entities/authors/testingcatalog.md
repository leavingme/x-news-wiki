---
type: "Author"
title: "TestingCatalog News 🗞 (@testingcatalog)"
description: "X/Twitter 作者 @testingcatalog。共收录 1 条 AI 精选推文。"
tags:
  - "x-news"
  - "author"
  - "author:testingcatalog"
timestamp: "2026-06-26T11:53:36.995Z"
x_handle: "testingcatalog"
x_author_name: "TestingCatalog News 🗞"
x_tweet_count: 1
x_first_seen: "2026-05-03T19:17:31.330Z"
x_last_seen: "2026-05-03T19:17:31.330Z"
---

# TestingCatalog News 🗞 (@testingcatalog)

X (Twitter) 作者 [@testingcatalog](https://x.com/testingcatalog)，共 **1** 条 AI 精选推文被收录到本知识库。

## 精选推文（按时间倒序）

- [Gemini iOS 端全新设计曝光：Google 正在把移动端做成 Agent 超级入口](../sources/news/2026-05-03/00__testingcatalog__gemini-ios-端全新设计曝光-google-正在把移动端做成-agent-超级入口.md) — 2026-05-03

## 互动统计

- **总点赞**: 1,361
- **总转发**: 45
- **总回复**: 65
- **总收藏**: 0
- **首次精选**: 2026-05-03
- **最近精选**: 2026-05-03
