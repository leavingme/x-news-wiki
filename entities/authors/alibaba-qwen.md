---
type: "entity"
title: "Qwen (@Alibaba_Qwen)"
description: "X/Twitter 作者 @Alibaba_Qwen。共收录 2 条 AI 精选推文。"
tags:
  - "x-news"
  - "author"
  - "author:Alibaba_Qwen"
timestamp: "2026-06-26T11:53:36.988Z"
x_handle: "Alibaba_Qwen"
x_author_name: "Qwen"
x_tweet_count: 2
x_first_seen: "2026-04-20T14:01:29.000Z"
x_last_seen: "2026-04-29T19:18:00.841Z"
type_okf: "Author"
created: "2026-04-20"
updated: "2026-06-26"
sha256: "1a6afeb9ae788694f1de3535673675a8b7904afb4542c3166ef1d9b094b99e50"
sources:
  - "/sources/highlights/2026-04-29/00__Alibaba_Qwen__qwen-发布-flashqla-线性注意力内核-2-3x-加速-边缘-agent-推理瓶颈松动.md"
  - "/sources/news/2026-04-20/00__Alibaba_Qwen__qwen3-6-max-preview-旗舰预览版的agent-coding能力再次跃升.md"
---

# Qwen (@Alibaba_Qwen)

X (Twitter) 作者 [@Alibaba_Qwen](https://x.com/Alibaba_Qwen)，共 **2** 条 AI 精选推文被收录到本知识库。

## 精选推文（按时间倒序）

- [Qwen 发布 FlashQLA：线性注意力内核 2-3x 加速，边缘 Agent 推理瓶颈松动](../sources/highlights/2026-04-29/00__Alibaba_Qwen__qwen-发布-flashqla-线性注意力内核-2-3x-加速-边缘-agent-推理瓶颈松动.md) — 2026-04-29
- [Qwen3.6-Max Preview：旗舰预览版的Agent Coding能力再次跃升](../sources/news/2026-04-20/00__Alibaba_Qwen__qwen3-6-max-preview-旗舰预览版的agent-coding能力再次跃升.md) — 2026-04-20

## 互动统计

- **总点赞**: 4,615
- **总转发**: 491
- **总回复**: 173
- **总收藏**: 0
- **首次精选**: 2026-04-20
- **最近精选**: 2026-04-29
