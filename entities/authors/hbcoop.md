---
type: "Author"
title: "Heather Cooper (@HBCoop_)"
description: "X/Twitter 作者 @HBCoop_。共收录 1 条 AI 精选推文。"
tags:
  - "x-news"
  - "author"
  - "author:HBCoop_"
timestamp: "2026-06-26T11:53:36.998Z"
x_handle: "HBCoop_"
x_author_name: "Heather Cooper"
x_tweet_count: 1
x_first_seen: "2026-05-07T19:18:11.732Z"
x_last_seen: "2026-05-07T19:18:11.732Z"
---

# Heather Cooper (@HBCoop_)

X (Twitter) 作者 [@HBCoop_](https://x.com/HBCoop_)，共 **1** 条 AI 精选推文被收录到本知识库。

## 精选推文（按时间倒序）

- [Seedance 2.0 多镜头叙事工程化：空间规则+结构化Prompt解决AI视频的「场景漂移」顽疾](../sources/news/2026-05-07/00__HBCoop___seedance-2-0-多镜头叙事工程化-空间规则-结构化prompt解决ai视频的-场景漂移-顽疾.md) — 2026-05-07

## 互动统计

- **总点赞**: 843
- **总转发**: 92
- **总回复**: 78
- **总收藏**: 0
- **首次精选**: 2026-05-07
- **最近精选**: 2026-05-07
