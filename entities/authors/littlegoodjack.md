---
type: "Author"
title: "小克 🌤 (@littlegoodjack)"
description: "X/Twitter 作者 @littlegoodjack。共收录 1 条 AI 精选推文。"
tags:
  - "x-news"
  - "author"
  - "author:littlegoodjack"
timestamp: "2026-06-27T08:40:40.186Z"
x_handle: "littlegoodjack"
x_author_name: "小克 🌤"
x_tweet_count: 1
x_first_seen: "2026-03-31T00:43:34.096Z"
x_last_seen: "2026-03-31T00:43:34.096Z"
---

# 小克 🌤 (@littlegoodjack)

X (Twitter) 作者 [@littlegoodjack](https://x.com/littlegoodjack)，共 **1** 条 AI 精选推文被收录到本知识库。

## 精选推文（按时间倒序）

- [Claude Code 进化：从代码编辑器到全栈操作系统](../sources/highlights/2026-03-31/00__littlegoodjack__claude-code-进化-从代码编辑器到全栈操作系统.md) — 2026-03-31

## 互动统计

- **总点赞**: 5
- **总转发**: 0
- **总回复**: 0
- **总收藏**: 0
- **首次精选**: 2026-03-31
- **最近精选**: 2026-03-31
