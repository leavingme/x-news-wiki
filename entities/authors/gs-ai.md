---
type: "entity"
title: "Genesis AI (@gs_ai_)"
description: "X/Twitter 作者 @gs_ai_。共收录 1 条 AI 精选推文。"
tags:
  - "x-news"
  - "author"
  - "author:gs_ai_"
timestamp: "2026-06-26T11:53:36.997Z"
x_handle: "gs_ai_"
x_author_name: "Genesis AI"
x_tweet_count: 1
x_first_seen: "2026-05-06T22:17:34.078Z"
x_last_seen: "2026-05-06T22:17:34.078Z"
type_okf: "Author"
created: "2026-05-06"
updated: "2026-06-26"
sha256: "882109f731c9df4b186554bc28a522a8366c5f037e59ec9adae46d4c64953bdb"
sources:
  - "/sources/news/2026-05-06/00__gs_ai___genesis-ai-沉默一年后发布-gene-26-5-从数据手套到仿真器的全栈机器人方案.md"
---

# Genesis AI (@gs_ai_)

X (Twitter) 作者 [@gs_ai_](https://x.com/gs_ai_)，共 **1** 条 AI 精选推文被收录到本知识库。

## 精选推文（按时间倒序）

- [Genesis AI 沉默一年后发布 GENE-26.5：从数据手套到仿真器的全栈机器人方案](../sources/news/2026-05-06/00__gs_ai___genesis-ai-沉默一年后发布-gene-26-5-从数据手套到仿真器的全栈机器人方案.md) — 2026-05-06

## 互动统计

- **总点赞**: 3,992
- **总转发**: 755
- **总回复**: 211
- **总收藏**: 0
- **首次精选**: 2026-05-06
- **最近精选**: 2026-05-06
