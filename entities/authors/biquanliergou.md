---
type: "Author"
title: "币圈李二狗 (@biquanliergou)"
description: "X/Twitter 作者 @biquanliergou。共收录 1 条 AI 精选推文。"
tags:
  - "x-news"
  - "author"
  - "author:biquanliergou"
timestamp: "2026-06-27T08:40:40.165Z"
x_handle: "biquanliergou"
x_author_name: "币圈李二狗"
x_tweet_count: 1
x_first_seen: "2026-03-13T12:42:48.380Z"
x_last_seen: "2026-03-13T12:42:48.380Z"
---

# 币圈李二狗 (@biquanliergou)

X (Twitter) 作者 [@biquanliergou](https://x.com/biquanliergou)，共 **1** 条 AI 精选推文被收录到本知识库。

## 精选推文（按时间倒序）

- [Crypto Agent的幻觉危机：QMD如何用「记忆减法」拯救交易AI](../sources/highlights/2026-03-13/88__biquanliergou__crypto-agent的幻觉危机-qmd如何用-记忆减法-拯救交易ai.md) — 2026-03-13 (priority 88)

## 互动统计

- **总点赞**: 57
- **总转发**: 11
- **总回复**: 47
- **总收藏**: 0
- **首次精选**: 2026-03-13
- **最近精选**: 2026-03-13
