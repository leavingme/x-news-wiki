---
type: "entity"
title: "NVIDIA AI (@NVIDIAAI)"
description: "X/Twitter 作者 @NVIDIAAI。共收录 1 条 AI 精选推文。"
tags:
  - "x-news"
  - "author"
  - "author:NVIDIAAI"
timestamp: "2026-06-26T11:53:36.992Z"
x_handle: "NVIDIAAI"
x_author_name: "NVIDIA AI"
x_tweet_count: 1
x_first_seen: "2026-04-28T22:17:50.494Z"
x_last_seen: "2026-04-28T22:17:50.494Z"
type_okf: "Author"
created: "2026-04-28"
updated: "2026-06-26"
sha256: "185dd29ad0f6375cd8efe4dec0d48183bb7921740616fd85e64fff13310e1d42"
sources:
  - "/sources/news/2026-04-28/00__NVIDIAAI__nvidia-nemotron-3-nano-30b参数多模态模型在端侧的场景定位.md"
---

# NVIDIA AI (@NVIDIAAI)

X (Twitter) 作者 [@NVIDIAAI](https://x.com/NVIDIAAI)，共 **1** 条 AI 精选推文被收录到本知识库。

## 精选推文（按时间倒序）

- [NVIDIA Nemotron 3 Nano：30B参数多模态模型在端侧的场景定位](../sources/news/2026-04-28/00__NVIDIAAI__nvidia-nemotron-3-nano-30b参数多模态模型在端侧的场景定位.md) — 2026-04-28

## 互动统计

- **总点赞**: 988
- **总转发**: 137
- **总回复**: 60
- **总收藏**: 0
- **首次精选**: 2026-04-28
- **最近精选**: 2026-04-28
