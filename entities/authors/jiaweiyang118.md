---
type: "Author"
title: "Jiawei Yang (@JiaweiYang118)"
description: "X/Twitter 作者 @JiaweiYang118。共收录 1 条 AI 精选推文。"
tags:
  - "x-news"
  - "author"
  - "author:JiaweiYang118"
timestamp: "2026-06-26T11:53:36.993Z"
x_handle: "JiaweiYang118"
x_author_name: "Jiawei Yang"
x_tweet_count: 1
x_first_seen: "2026-05-01T07:17:35.193Z"
x_last_seen: "2026-05-01T07:17:35.193Z"
---

# Jiawei Yang (@JiaweiYang118)

X (Twitter) 作者 [@JiaweiYang118](https://x.com/JiaweiYang118)，共 **1** 条 AI 精选推文被收录到本知识库。

## 精选推文（按时间倒序）

- [单步扩散突破 0.75 FID 极限：直接优化目标函数的算法红利来了](../sources/highlights/2026-05-01/00__JiaweiYang118__单步扩散突破-0-75-fid-极限-直接优化目标函数的算法红利来了.md) — 2026-05-01

## 互动统计

- **总点赞**: 675
- **总转发**: 115
- **总回复**: 43
- **总收藏**: 0
- **首次精选**: 2026-05-01
- **最近精选**: 2026-05-01
