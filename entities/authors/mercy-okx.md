---
type: "entity"
title: "Mercy (@Mercy_okx)"
description: "X/Twitter 作者 @Mercy_okx。共收录 1 条 AI 精选推文。"
tags:
  - "x-news"
  - "author"
  - "author:Mercy_okx"
timestamp: "2026-06-26T11:53:36.966Z"
x_handle: "Mercy_okx"
x_author_name: "Mercy"
x_tweet_count: 1
x_first_seen: "2026-03-24T00:42:53.999Z"
x_last_seen: "2026-03-24T00:42:53.999Z"
type_okf: "Author"
created: "2026-03-24"
updated: "2026-06-26"
sha256: "7115666a1d2c1bb0d2602a3bc5117738274761fa7f5d983226f9a706c99d5a44"
sources:
  - "/sources/highlights/2026-03-24/00__Mercy_okx__agentic-economy-临界点-okx-agentic-wallet-正在打通-ai-自动化执行的最后一公里.md"
---

# Mercy (@Mercy_okx)

X (Twitter) 作者 [@Mercy_okx](https://x.com/Mercy_okx)，共 **1** 条 AI 精选推文被收录到本知识库。

## 精选推文（按时间倒序）

- [Agentic Economy 临界点：OKX Agentic Wallet 正在打通 AI 自动化执行的最后一公里](../sources/highlights/2026-03-24/00__Mercy_okx__agentic-economy-临界点-okx-agentic-wallet-正在打通-ai-自动化执行的最后一公里.md) — 2026-03-24

## 互动统计

- **总点赞**: 33
- **总转发**: 2
- **总回复**: 16
- **总收藏**: 0
- **首次精选**: 2026-03-24
- **最近精选**: 2026-03-24
