---
type: "Author"
title: "Zhijian Liu (@zhijianliu_)"
description: "X/Twitter 作者 @zhijianliu_。共收录 1 条 AI 精选推文。"
tags:
  - "x-news"
  - "author"
  - "author:zhijianliu_"
timestamp: "2026-06-27T08:40:40.212Z"
x_handle: "zhijianliu_"
x_author_name: "Zhijian Liu"
x_tweet_count: 1
x_first_seen: "2026-05-06T10:17:45.695Z"
x_last_seen: "2026-05-06T10:17:45.695Z"
---

# Zhijian Liu (@zhijianliu_)

X (Twitter) 作者 [@zhijianliu_](https://x.com/zhijianliu_)，共 **1** 条 AI 精选推文被收录到本知识库。

## 精选推文（按时间倒序）

- [DFlash 为 Gemma 4 提速 6 倍：开源推理优化工具瞄准端侧部署效率瓶颈](../sources/news/2026-05-06/00__zhijianliu___dflash-为-gemma-4-提速-6-倍-开源推理优化工具瞄准端侧部署效率瓶颈.md) — 2026-05-06

## 互动统计

- **总点赞**: 425
- **总转发**: 47
- **总回复**: 27
- **总收藏**: 0
- **首次精选**: 2026-05-06
- **最近精选**: 2026-05-06
