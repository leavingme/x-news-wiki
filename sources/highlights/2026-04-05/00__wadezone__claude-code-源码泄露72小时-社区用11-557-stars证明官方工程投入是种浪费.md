---
type: "X Tweet"
title: "Claude Code 源码泄露72小时：社区用11,557 Stars证明官方工程投入是种浪费"
description: "**[Anthropic雇了几百工程师做的工具，社区三天做出了更好用的版本]**  Claude Code 源码意外泄露后，Anthropic 官方保持沉默。开发者社区在72小时内完成拆解、改造、去限制、加搜索、修bug，最终产出开源版本——这个过程本应是官方工程师花几个月才能排上日程的工作。  核心信号：开源社区的分布式试错正在以数量级优势碾压官方集中式开发。当一个工具的代码泄露后社区比官方更快"
resource: "https://x.com/i/status/2040387896541380876"
tags:
  - "x-news"
  - "highlights"
  - "author:wadezone"
timestamp: "2026-04-05T00:42:57.267Z"
x_tweet_id: "2040387896541380876"
x_author: "Koda"
x_handle: "wadezone"
x_created_at: "2026-04-04T11:15:57.000Z"
x_engagement:
  likes: 3
  retweets: 0
  replies: 2
  bookmarks: 0
  views: 0
x_selected: 2
x_priority: 0
x_related_topic: "claude-code-leaked-architecture"
x_source: "crawler"
---

# Claude Code 源码泄露72小时：社区用11,557 Stars证明官方工程投入是种浪费

## 摘要

**[Anthropic雇了几百工程师做的工具，社区三天做出了更好用的版本]**

Claude Code 源码意外泄露后，Anthropic 官方保持沉默。开发者社区在72小时内完成拆解、改造、去限制、加搜索、修bug，最终产出开源版本——这个过程本应是官方工程师花几个月才能排上日程的工作。

核心信号：开源社区的分布式试错正在以数量级优势碾压官方集中式开发。当一个工具的代码泄露后社区比官方更快产出更好结果，这不仅是技术问题，而是对 Anthropic 工程路线图的根本性质疑。

## 原文

Claude Code 源码意外泄露。

Anthropic 官方：保持沉默。

开发者社区：72小时内拆解、改造、重新打包，直接开源。

他们做了什么？把官方版本里的限制去掉，加上了搜索功能，修了一堆bug，整个代码结构重新整理了一遍——基本上是官方工程师要花好几个月才能排上日程的事。

3天，11,557 个 Star。

这就是现实：Anthropic 雇了几百个工程师，做了一个工具，结果社区用三天做出了一个更好用的版本。

官方到底在做什么？还是说，开发者社区才是真正推动这个产品的人？


## 元信息

- **作者**: Koda ([@wadezone](https://x.com/wadezone))
- **发布时间**: 2026-04-04T11:15:57.000Z
- **抓取时间**: 2026-04-05T00:42:57.267Z
- **精选类别**: ✨ 亮点
- **优先级**: 0
- **互动**: ❤️ 3 · 🔄 0 · 💬 2 · 🔖 0 · 👁 0
- **关联主题**: [claude-code-leaked-architecture](/concepts/claude-code-leaked-architecture.md)

## Citations

[1] [Tweet on X](https://x.com/i/status/2040387896541380876) — @wadezone (Koda)
[2] [Author page](/entities/authors/wadezone.md)
[3] [Related topic](/concepts/claude-code-leaked-architecture.md)
