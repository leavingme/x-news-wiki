---
type: "X Tweet"
title: "前端审美榜 Claude Opus 4.8 登顶：主观排名正在替代 Benchmark 成为体感标尺"
description: "**Claude Opus 4.8 在前端审美主观榜中位列第一，Kimi 2.6 反超 GPT 5.5 排第二，国产 GLM 5.1 也进入前五。**  - 向阳乔木与 Tom 直播中给出的完整排序：Claude Opus 4.8 > Kimi 2.6 > GPT 5.5 > DeepSeek V4 Pro > GLM 5.1 > DeepSeek V4 Flash - 明确声明「都是大家的主观看"
resource: "https://x.com/i/status/2063264223879389431"
tags:
  - "x-news"
  - "highlights"
  - "author:vista8"
timestamp: "2026-06-06T16:18:38.911Z"
x_tweet_id: "2063264223879389431"
x_author: "向阳乔木"
x_handle: "vista8"
x_created_at: "2026-06-06T14:18:19.000Z"
x_engagement:
  likes: 1
  retweets: 0
  replies: 8
  bookmarks: 0
  views: 0
x_selected: 2
x_priority: 0
x_related_topic: "design-model-evaluation"
x_source: "crawler"
---

# 前端审美榜 Claude Opus 4.8 登顶：主观排名正在替代 Benchmark 成为体感标尺

## 摘要

**Claude Opus 4.8 在前端审美主观榜中位列第一，Kimi 2.6 反超 GPT 5.5 排第二，国产 GLM 5.1 也进入前五。**

- 向阳乔木与 Tom 直播中给出的完整排序：Claude Opus 4.8 > Kimi 2.6 > GPT 5.5 > DeepSeek V4 Pro > GLM 5.1 > DeepSeek V4 Flash
- 明确声明「都是大家的主观看法，没有 Benchmark」——但排序本身正在被当作选型依据传播
- Kimi 2.6 在设计审美上挤进第二，是国产模型在前端场景的少见高位


gakki 锐评：在「设计」这种主观审美任务上，Benchmark 本就乏力。榜单本身比排名更重要——它揭示了行业正在用「圈子里的体感」替代「论文里的分数」。但样本仅来自向阳乔木与 Tom 二人对话，外推性有限，别把个人偏好当真相。

## 原文

跟Tom聊完了，直播中讨论的大模型前端审美主观排名：

Claude opus 4.8 &gt; kimi2.6 &gt; GPT 5.5 &gt; Deepseek v4 pro &gt; GLM 5.1&gt; deepseek v4 flash

不知道和你的实际使用体感一样不一样。

声明：都是大家的主观看法，没有Benchmark。


## 元信息

- **作者**: 向阳乔木 ([@vista8](https://x.com/vista8))
- **发布时间**: 2026-06-06T14:18:19.000Z
- **抓取时间**: 2026-06-06T16:18:38.911Z
- **精选类别**: ✨ 亮点
- **优先级**: 0
- **互动**: ❤️ 1 · 🔄 0 · 💬 8 · 🔖 0 · 👁 0
- **关联主题**: [design-model-evaluation](/concepts/design-model-evaluation.md)

## Citations

[1] [Tweet on X](https://x.com/i/status/2063264223879389431) — @vista8 (向阳乔木)
[2] [Author page](/entities/authors/vista8.md)
[3] [Related topic](/concepts/design-model-evaluation.md)
