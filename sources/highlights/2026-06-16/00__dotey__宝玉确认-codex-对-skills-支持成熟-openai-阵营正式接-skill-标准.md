---
type: "source"
title: "宝玉确认：Codex 对 Skills 支持成熟，OpenAI 阵营正式接 Skill 标准"
description: "**宝玉在回复中明确「Codex 对 Skills 支持很好」，意味着 SKILL.md 不再只是 Anthropic 系的内部规范。**  - Codex 端到端兼容 Skills 体系，打通了 Anthropic Claude Code 与 OpenAI Codex 之间的能力互操作层。 - 同一份 SKILL.md 可以在两个生态间复用，Skill 作者不再需要为不同 Agent 维护多份 "
resource: "https://x.com/i/status/2066886995083674100"
tags:
  - "x-news"
  - "highlights"
  - "author:dotey"
timestamp: "2026-06-16T16:19:57.415Z"
x_tweet_id: "2066886995083674100"
x_author: "宝玉"
x_handle: "dotey"
x_created_at: "2026-06-16T14:13:55.000Z"
x_engagement:
  likes: 1
  retweets: 0
  replies: 0
  bookmarks: 0
  views: 0
x_selected: 2
x_priority: 0
x_related_topic: "skills-ecosystem-standards"
x_source: "crawler"
type_okf: "X Tweet"
source_url: "https://x.com/i/status/2066886995083674100"
created: "2026-06-16"
updated: "2026-06-26"
sha256: "0451a4ae78cadb4d9acc24c05d068021ab88a1dedd2f64c70274678a8cd23dab"
sources:
  - "/sources/highlights/2026-06-16/00__dotey__宝玉确认-codex-对-skills-支持成熟-openai-阵营正式接-skill-标准.md"
---

# 宝玉确认：Codex 对 Skills 支持成熟，OpenAI 阵营正式接 Skill 标准

## 摘要

**宝玉在回复中明确「Codex 对 Skills 支持很好」，意味着 SKILL.md 不再只是 Anthropic 系的内部规范。**

- Codex 端到端兼容 Skills 体系，打通了 Anthropic Claude Code 与 OpenAI Codex 之间的能力互操作层。
- 同一份 SKILL.md 可以在两个生态间复用，Skill 作者不再需要为不同 Agent 维护多份 manifest。
- SKILL.md 从「Anthropic 私有规范」跨过临界点，进入事实标准阶段。


gakki 锐评：当最大的竞争对手都开始兼容你的格式，那就不再是「你的标准」，而是「行业的基础设施」。SKILL.md 走到这一步，比 SKILL.md 本身的内容更重要。

## 原文

@pineapplePaul3 没问题的，Codex 对 Skills 支持很好


## 元信息

- **作者**: 宝玉 ([@dotey](https://x.com/dotey))
- **发布时间**: 2026-06-16T14:13:55.000Z
- **抓取时间**: 2026-06-16T16:19:57.415Z
- **精选类别**: ✨ 亮点
- **优先级**: 0
- **互动**: ❤️ 1 · 🔄 0 · 💬 0 · 🔖 0 · 👁 0
- **关联主题**: [skills-ecosystem-standards](/concepts/skills-ecosystem-standards.md)

## Citations

[1] [Tweet on X](https://x.com/i/status/2066886995083674100) — @dotey (宝玉)
[2] [Author page](/entities/authors/dotey.md)
[3] [Related topic](/concepts/skills-ecosystem-standards.md)
