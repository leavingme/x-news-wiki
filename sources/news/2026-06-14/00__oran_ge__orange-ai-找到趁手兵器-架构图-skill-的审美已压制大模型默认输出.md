---
type: "source"
title: "Orange AI 找到趁手兵器：架构图 Skill 的审美已压制大模型默认输出"
description: "**「审美+结构双重约束」的内容生成正在脱离\"让大模型自由发挥\"范式。**  - Orange AI 推荐一个架构图 Skill,认为比大模型默认生成的好用太多 - 与 Orange AI 此前蒸馏的「橙线插画」Skill 同向：专业设计工序正在被封装成可调用 Skill - 呼应 DESIGN.md / MarkItDown 这类「结构化设计协议」的整体趋势   gakki 锐评：单条推荐不构成"
resource: "https://x.com/i/status/2065973931169956282"
tags:
  - "x-news"
  - "news"
  - "author:oran_ge"
timestamp: "2026-06-14T04:18:25.891Z"
x_tweet_id: "2065973931169956282"
x_author: "Orange AI"
x_handle: "oran_ge"
x_created_at: "2026-06-14T01:45:43.000Z"
x_engagement:
  likes: 25
  retweets: 5
  replies: 1
  bookmarks: 0
  views: 0
x_selected: 1
x_priority: 0
x_related_topic:

x_source: "crawler"
type_okf: "X Tweet"
source_url: "https://x.com/i/status/2065973931169956282"
created: "2026-06-14"
updated: "2026-06-26"
sha256: "b81ddf242332dbe74ba20c166a5d1b410aeb8ac81a64bca5f1d89b4b0c605e56"
sources:
  - "/sources/news/2026-06-14/00__oran_ge__orange-ai-找到趁手兵器-架构图-skill-的审美已压制大模型默认输出.md"
---

# Orange AI 找到趁手兵器：架构图 Skill 的审美已压制大模型默认输出

## 摘要

**「审美+结构双重约束」的内容生成正在脱离"让大模型自由发挥"范式。**

- Orange AI 推荐一个架构图 Skill,认为比大模型默认生成的好用太多
- 与 Orange AI 此前蒸馏的「橙线插画」Skill 同向：专业设计工序正在被封装成可调用 Skill
- 呼应 DESIGN.md / MarkItDown 这类「结构化设计协议」的整体趋势


gakki 锐评：单条推荐不构成趋势,但配合 Orange AI 的"六条原则"和橙线插画——设计工序 Skill 化已过临界点,接下来是垂直行业工序的批量复制。

## 原文

一直想找个画架构图的趁手的兵器
这个 skill 很好
比大模型默认的审美好多了


## 元信息

- **作者**: Orange AI ([@oran_ge](https://x.com/oran_ge))
- **发布时间**: 2026-06-14T01:45:43.000Z
- **抓取时间**: 2026-06-14T04:18:25.891Z
- **精选类别**: 📰 头条
- **优先级**: 0
- **互动**: ❤️ 25 · 🔄 5 · 💬 1 · 🔖 0 · 👁 0

## Citations

[1] [Tweet on X](https://x.com/i/status/2065973931169956282) — @oran_ge (Orange AI)
[2] [Author page](/entities/authors/oran-ge.md)
