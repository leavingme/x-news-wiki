---
type: "source"
title: "yetone 一句话点破\"AI Coding 怀旧\"元命题：制品没变，所以才有比较"
description: "**Vibe Coding 焦虑的根源不是模型能力，是自我价值感的锚点被复制。**  - 我们不怀念打孔纸→因为制品从打孔纸变成了代码，介质变了 - 我们怀念手写代码→因为 AI Coding 制品还是源代码，介质没变 - 制品同质 = 比较自然发生 = \"AI 写的就不如我\"的情感来源  锐评：这是 Vibe Coding 争议的底层心法。承认这层心理机制（自我价值感锚点被复制），才能不把工程问"
resource: "https://x.com/i/status/2069336002662113392"
tags:
  - "x-news"
  - "news"
  - "author:yetone"
timestamp: "2026-06-23T10:18:39.257Z"
x_tweet_id: "2069336002662113392"
x_author: "yetone"
x_handle: "yetone"
x_created_at: "2026-06-23T08:25:24.000Z"
x_engagement:
  likes: 7
  retweets: 0
  replies: 0
  bookmarks: 0
  views: 0
x_selected: 1
x_priority: 0
x_related_topic:

x_source: "crawler"
type_okf: "X Tweet"
source_url: "https://x.com/i/status/2069336002662113392"
created: "2026-06-23"
updated: "2026-06-26"
sha256: "0aca08b980e8ed9c403c1fe342b35a66b875e1599ce6dbf3d8f63bb59351b584"
sources:
  - "/sources/news/2026-06-23/00__yetone__yetone-一句话点破-ai-coding-怀旧-元命题-制品没变-所以才有比较.md"
---

# yetone 一句话点破"AI Coding 怀旧"元命题：制品没变，所以才有比较

## 摘要

**Vibe Coding 焦虑的根源不是模型能力，是自我价值感的锚点被复制。**

- 我们不怀念打孔纸→因为制品从打孔纸变成了代码，介质变了
- 我们怀念手写代码→因为 AI Coding 制品还是源代码，介质没变
- 制品同质 = 比较自然发生 = "AI 写的就不如我"的情感来源

锐评：这是 Vibe Coding 争议的底层心法。承认这层心理机制（自我价值感锚点被复制），才能不把工程问题搞成身份战争。判断力稀缺是真的，但表达上不必非此即彼。

## 原文

@ccs668899 你不去回想它们是因为现在的制品是代码，以前的制品是打孔纸。你会去怀念手写代码，是因为现在AI Coding的制品是源代码，以前的制品也是源代码。都是一样的东西，所以自然而然有了比较。


## 元信息

- **作者**: yetone ([@yetone](https://x.com/yetone))
- **发布时间**: 2026-06-23T08:25:24.000Z
- **抓取时间**: 2026-06-23T10:18:39.257Z
- **精选类别**: 📰 头条
- **优先级**: 0
- **互动**: ❤️ 7 · 🔄 0 · 💬 0 · 🔖 0 · 👁 0

## Citations

[1] [Tweet on X](https://x.com/i/status/2069336002662113392) — @yetone (yetone)
[2] [Author page](/entities/authors/yetone.md)
