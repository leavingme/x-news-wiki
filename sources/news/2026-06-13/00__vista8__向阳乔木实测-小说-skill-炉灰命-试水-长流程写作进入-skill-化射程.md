---
type: "source"
title: "向阳乔木实测「小说 Skill」:《炉灰命》试水,长流程写作进入 Skill 化射程"
description: "**小说这类长流程、强结构的内容创作,正在被拆解成可调用的 Skill。**  - 向阳乔木测试「小说 Skill」,用作品《炉灰命》做载体 - 与此前开源的 qiaomu-book-script(书籍解读 Skill)同向——中文长内容创作的 Skill 化推进中 - 标志着 SKILL.md 生态从代码 / 办公向\"长篇创作\"扩张   gakki 锐评:单次测试不证伪可行性,但 Skill 化"
resource: "https://x.com/i/status/2065797575022968961"
tags:
  - "x-news"
  - "news"
  - "author:vista8"
timestamp: "2026-06-13T16:18:21.276Z"
x_tweet_id: "2065797575022968961"
x_author: "向阳乔木"
x_handle: "vista8"
x_created_at: "2026-06-13T14:04:57.000Z"
x_engagement:
  likes: 15
  retweets: 2
  replies: 5
  bookmarks: 0
  views: 0
x_selected: 1
x_priority: 0
x_related_topic: "skills-ecosystem-standards"
x_source: "crawler"
type_okf: "X Tweet"
source_url: "https://x.com/i/status/2065797575022968961"
created: "2026-06-13"
updated: "2026-06-26"
sha256: "94be350e083b8615e3555462428b531f861fd82a1ed8ca2432163d7e0a3a8dee"
sources:
  - "/sources/news/2026-06-13/00__vista8__向阳乔木实测-小说-skill-炉灰命-试水-长流程写作进入-skill-化射程.md"
---

# 向阳乔木实测「小说 Skill」:《炉灰命》试水,长流程写作进入 Skill 化射程

## 摘要

**小说这类长流程、强结构的内容创作,正在被拆解成可调用的 Skill。**

- 向阳乔木测试「小说 Skill」,用作品《炉灰命》做载体
- 与此前开源的 qiaomu-book-script(书籍解读 Skill)同向——中文长内容创作的 Skill 化推进中
- 标志着 SKILL.md 生态从代码 / 办公向"长篇创作"扩张


gakki 锐评:单次测试不证伪可行性,但 Skill 化方向已无悬念——接下来是哪些创作工序能熬过「复用次数」检验,哪些只是演示型 demo。

## 原文

小说Skill测试《炉灰命》


## 元信息

- **作者**: 向阳乔木 ([@vista8](https://x.com/vista8))
- **发布时间**: 2026-06-13T14:04:57.000Z
- **抓取时间**: 2026-06-13T16:18:21.276Z
- **精选类别**: 📰 头条
- **优先级**: 0
- **互动**: ❤️ 15 · 🔄 2 · 💬 5 · 🔖 0 · 👁 0
- **关联主题**: [skills-ecosystem-standards](/concepts/skills-ecosystem-standards.md)

## Citations

[1] [Tweet on X](https://x.com/i/status/2065797575022968961) — @vista8 (向阳乔木)
[2] [Author page](/entities/authors/vista8.md)
[3] [Related topic](/concepts/skills-ecosystem-standards.md)
