---
type: "X Tweet"
title: "Hermes Agent 自进化：prompt工程正在被prompt自己取代"
description: "**一句话核心判断：** ICLR 2026 Oral 论文 GEPA 用 35 倍更少的数据超越 RLHF，意味着「人工调 prompt」正在被「模型自己优化 prompt」彻底替代——这是 prompt 工程职业存亡的分水岭。  **信息增量：** - Nous Research 开源 hermes-agent-self-evolution，核心引擎 GEPA 获 ICLR 2026 Oral"
resource: "https://x.com/i/status/2043119101028274268"
tags:
  - "x-news"
  - "news"
  - "author:KKaWSB"
timestamp: "2026-04-12T00:44:13.314Z"
x_tweet_id: "2043119101028274268"
x_author: "KK.aWSB"
x_handle: "KKaWSB"
x_created_at: "2026-04-12T00:08:47.000Z"
x_engagement:
  likes: 1552
  retweets: 197
  replies: 20
  bookmarks: 0
  views: 0
x_selected: 1
x_priority: 0
x_related_topic: "hermes-agent-self-evolution"
x_source: "crawler"
---

# Hermes Agent 自进化：prompt工程正在被prompt自己取代

## 摘要

**一句话核心判断：** ICLR 2026 Oral 论文 GEPA 用 35 倍更少的数据超越 RLHF，意味着「人工调 prompt」正在被「模型自己优化 prompt」彻底替代——这是 prompt 工程职业存亡的分水岭。

**信息增量：**
- Nous Research 开源 hermes-agent-self-evolution，核心引擎 GEPA 获 ICLR 2026 Oral
- 对比强化学习，GEPA 数据效率提升 35 倍，性能反而高出 20 个百分点
- 意味着 Agent 不再依赖人类手工注入 system prompt，而是通过学习反馈闭环自主「长出」最优行为模式

**影响范围：** 所有依赖 prompt 工程师 / AI 训镜师岗位的团队需要重新评估人力配置。

gakki：「别再手调 system prompt」这句话听起来像标题党，但 GEPA 的数据不是虚标。如果 35x 数据效率和 20% 性能优势被复现，prompt 工程这个岗位的半衰期可能比前端还短。

## 原文

Nous Research开源了hermes-agent-self-evolution——一个让AI agent自己进化自己prompt的代码库，

背后引擎叫GEPA，ICLR 2026 Oral，比强化学习少用35倍数据，效果还高出20个百分点。

别再手调system prompt了。prompt工程这件事，正在被prompt自己取代。

https://t.co/zNOOqeP4Lg


## 元信息

- **作者**: KK.aWSB ([@KKaWSB](https://x.com/KKaWSB))
- **发布时间**: 2026-04-12T00:08:47.000Z
- **抓取时间**: 2026-04-12T00:44:13.314Z
- **精选类别**: 📰 头条
- **优先级**: 0
- **互动**: ❤️ 1552 · 🔄 197 · 💬 20 · 🔖 0 · 👁 0
- **关联主题**: [hermes-agent-self-evolution](/concepts/hermes-agent-self-evolution.md)

## Citations

[1] [Tweet on X](https://x.com/i/status/2043119101028274268) — @KKaWSB (KK.aWSB)
[2] [Author page](/entities/authors/kkawsb.md)
[3] [Related topic](/concepts/hermes-agent-self-evolution.md)
