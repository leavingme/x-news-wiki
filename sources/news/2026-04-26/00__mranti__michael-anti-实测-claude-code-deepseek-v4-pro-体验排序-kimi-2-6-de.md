---
type: "source"
title: "Michael Anti 实测：Claude Code + Deepseek V4 Pro 体验排序 Kimi 2.6 > Deepseek V4 Pro > Kimi 2.5"
description: "**[Claude Code 兼容多模型已成现实，但 Deepseek V4 Pro 性价比存疑]**  - Michael Anti（Antino 创始人）实测反馈：Claude Code 接入 Deepseek V4 Pro 后手感排序为 Kimi 2.6 > Deepseek V4 Pro > Kimi 2.5，Deepseek V4 Pro 处于中间位置而非绝对领先。 - 这说明 Clau"
resource: "https://x.com/i/status/2048459470150488181"
tags:
  - "x-news"
  - "news"
  - "author:mranti"
timestamp: "2026-04-26T19:18:37.550Z"
x_tweet_id: "2048459470150488181"
x_author: "Michael Anti"
x_handle: "mranti"
x_created_at: "2026-04-26T17:49:30.000Z"
x_engagement:
  likes: 12
  retweets: 0
  replies: 5
  bookmarks: 0
  views: 0
x_selected: 1
x_priority: 0
x_related_topic:

x_source: "crawler"
type_okf: "X Tweet"
source_url: "https://x.com/i/status/2048459470150488181"
created: "2026-04-26"
updated: "2026-06-26"
sha256: "b0fc266ef4cb970e6aaec0bd9c0daf9b069e4bb2cdbb3232406e028e284d385a"
sources:
  - "/sources/news/2026-04-26/00__mranti__michael-anti-实测-claude-code-deepseek-v4-pro-体验排序-kimi-2-6-de.md"
---

# Michael Anti 实测：Claude Code + Deepseek V4 Pro 体验排序 Kimi 2.6 > Deepseek V4 Pro > Kimi 2.5

## 摘要

**[Claude Code 兼容多模型已成现实，但 Deepseek V4 Pro 性价比存疑]**

- Michael Anti（Antino 创始人）实测反馈：Claude Code 接入 Deepseek V4 Pro 后手感排序为 Kimi 2.6 > Deepseek V4 Pro > Kimi 2.5，Deepseek V4 Pro 处于中间位置而非绝对领先。
- 这说明 Claude Code 的多模型路由能力已具备一定可用性，但模型间体验差距仍然显著。
- 国产模型的 Claude Code 集成度正在成为新的竞争维度。

[gakki] Claude Code 正在变成模型评测的实战考场——纯跑分没用，能在 Claude Code 里用得顺手才是真本事。

## 原文

我在用Claude Code接Deepseek V4 Pro，刚刚开始试，手感是 Kimi 2.6 &gt; Deepseek V4 Pro &gt; Kimi 2.5。不知道大家有没同感？


## 元信息

- **作者**: Michael Anti ([@mranti](https://x.com/mranti))
- **发布时间**: 2026-04-26T17:49:30.000Z
- **抓取时间**: 2026-04-26T19:18:37.550Z
- **精选类别**: 📰 头条
- **优先级**: 0
- **互动**: ❤️ 12 · 🔄 0 · 💬 5 · 🔖 0 · 👁 0

## Citations

[1] [Tweet on X](https://x.com/i/status/2048459470150488181) — @mranti (Michael Anti)
[2] [Author page](/entities/authors/mranti.md)
