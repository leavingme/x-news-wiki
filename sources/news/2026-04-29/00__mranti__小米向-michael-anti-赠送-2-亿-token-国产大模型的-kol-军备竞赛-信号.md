---
type: "source"
title: "小米向 Michael Anti 赠送 2 亿 Token：国产大模型的「KOL 军备竞赛」信号"
description: "**小米 AI 主动向高影响力用户赠送大量 Token 用于对标测试，暴露了国产大模型在 C 端认知战中的焦虑。**  - Michael Anti 是中文圈最有影响力的科技意见领袖之一，其对比测试结果（MiMo vs DeepSeek V4 Pro）将直接影响开发者社区的模型选型判断。 - 2 亿 Token 的赠送规模暗示小米对自家模型的信心——或者至少愿意承担被公开对比的风险。 - 这种「K"
resource: "https://x.com/i/status/2049447119258534205"
tags:
  - "x-news"
  - "news"
  - "author:mranti"
timestamp: "2026-04-29T13:19:30.014Z"
x_tweet_id: "2049447119258534205"
x_author: "Michael Anti"
x_handle: "mranti"
x_created_at: "2026-04-29T11:14:04.000Z"
x_engagement:
  likes: 30
  retweets: 1
  replies: 12
  bookmarks: 0
  views: 0
x_selected: 1
x_priority: 0
x_related_topic:

x_source: "crawler"
type_okf: "X Tweet"
source_url: "https://x.com/i/status/2049447119258534205"
created: "2026-04-29"
updated: "2026-06-26"
sha256: "ac2b366563c3184f1ace09a3cd002277f3e99217702f26e7bbfebee86bd8fd21"
sources:
  - "/sources/news/2026-04-29/00__mranti__小米向-michael-anti-赠送-2-亿-token-国产大模型的-kol-军备竞赛-信号.md"
---

# 小米向 Michael Anti 赠送 2 亿 Token：国产大模型的「KOL 军备竞赛」信号

## 摘要

**小米 AI 主动向高影响力用户赠送大量 Token 用于对标测试，暴露了国产大模型在 C 端认知战中的焦虑。**

- Michael Anti 是中文圈最有影响力的科技意见领袖之一，其对比测试结果（MiMo vs DeepSeek V4 Pro）将直接影响开发者社区的模型选型判断。
- 2 亿 Token 的赠送规模暗示小米对自家模型的信心——或者至少愿意承担被公开对比的风险。
- 这种「KOL 评测先行」的推广策略正在成为国产大模型的标准打法，类似手机圈的媒体机分发。

**gakki 锐评：** 当模型能力差距收窄到一定程度，「谁先占领意见领袖的认知」就变成了真正的竞争维度。小米这步棋不在于 Token 成本，在于如果 Michael Anti 的测试结论是「MiMo 不输 DeepSeek」，那对小米 AI 品牌的杠杆效应远超千万级广告投放。

## 原文

感谢，收到了小米赠送的2亿Token，我测试下，和Deepseek V4 Pro对比对比。 https://t.co/qqlfWAJsmb


## 元信息

- **作者**: Michael Anti ([@mranti](https://x.com/mranti))
- **发布时间**: 2026-04-29T11:14:04.000Z
- **抓取时间**: 2026-04-29T13:19:30.014Z
- **精选类别**: 📰 头条
- **优先级**: 0
- **互动**: ❤️ 30 · 🔄 1 · 💬 12 · 🔖 0 · 👁 0

## Citations

[1] [Tweet on X](https://x.com/i/status/2049447119258534205) — @mranti (Michael Anti)
[2] [Author page](/entities/authors/mranti.md)
