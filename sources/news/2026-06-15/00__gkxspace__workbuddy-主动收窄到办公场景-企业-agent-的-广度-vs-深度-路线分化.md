---
type: "X Tweet"
title: "WorkBuddy 主动收窄到办公场景：企业 Agent 的'广度 vs 深度'路线分化"
description: "**WorkBuddy 的办公定位，是企业 Agent 从'通用平台'转向'场景专家'的标志性信号。**  1. 余温指出 WorkBuddy 主动定位办公场景，放弃通用 Agent 的广度 2. 与'通用 Agent 框架 + Skills 生态'路径形成对照——企业服务真正需要的是深度而非广度 3. 印证腾讯 AI 在 to B 端的差异化：不与字节、火山在通用能力上硬碰，而是切办公垂直  锐"
resource: "https://x.com/i/status/2066525515331047485"
tags:
  - "x-news"
  - "news"
  - "author:gkxspace"
timestamp: "2026-06-15T16:18:49.181Z"
x_tweet_id: "2066525515331047485"
x_author: "余温"
x_handle: "gkxspace"
x_created_at: "2026-06-15T14:17:31.000Z"
x_engagement:
  likes: 0
  retweets: 0
  replies: 0
  bookmarks: 0
  views: 0
x_selected: 1
x_priority: 0
x_related_topic: "enterprise-agent-product"
x_source: "crawler"
---

# WorkBuddy 主动收窄到办公场景：企业 Agent 的'广度 vs 深度'路线分化

## 摘要

**WorkBuddy 的办公定位，是企业 Agent 从'通用平台'转向'场景专家'的标志性信号。**

1. 余温指出 WorkBuddy 主动定位办公场景，放弃通用 Agent 的广度
2. 与'通用 Agent 框架 + Skills 生态'路径形成对照——企业服务真正需要的是深度而非广度
3. 印证腾讯 AI 在 to B 端的差异化：不与字节、火山在通用能力上硬碰，而是切办公垂直

锐评：办公场景看似窄，实则是企业采购决策的'舒适区'——WorkBuddy 选了一条 ROI 最容易讲清楚的路径；问题在于，'办公'仍偏宽，真正的胜负手还要看它能下沉到多具体的岗位角色。

## 原文

@Sqrt241027 workbuddy定位更偏向办公


## 元信息

- **作者**: 余温 ([@gkxspace](https://x.com/gkxspace))
- **发布时间**: 2026-06-15T14:17:31.000Z
- **抓取时间**: 2026-06-15T16:18:49.181Z
- **精选类别**: 📰 头条
- **优先级**: 0
- **互动**: ❤️ 0 · 🔄 0 · 💬 0 · 🔖 0 · 👁 0
- **关联主题**: [enterprise-agent-product](/concepts/enterprise-agent-product.md)

## Citations

[1] [Tweet on X](https://x.com/i/status/2066525515331047485) — @gkxspace (余温)
[2] [Author page](/entities/authors/gkxspace.md)
[3] [Related topic](/concepts/enterprise-agent-product.md)
