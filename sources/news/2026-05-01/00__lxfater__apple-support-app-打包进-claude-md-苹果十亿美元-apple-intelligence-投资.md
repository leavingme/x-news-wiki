---
type: "source"
title: "Apple Support App 打包进 Claude.md：苹果十亿美元 Apple Intelligence 投资正在被第三方工具链静默替代"
description: "**苹果内部工程团队已实际采用 Claude Code 工作流，而非自研 Apple Intelligence。**  - Apple Support App 的发布包中被发现包含 Claude.md 文件，这是 Claude Code 的标准项目配置标识 - 苹果此前投入超 10 亿美元建设 Apple Intelligence 基础设施，但一线工程团队的实际工具选择已发生偏移 - 这是继多家大"
resource: "https://x.com/i/status/2050240636020732015"
tags:
  - "x-news"
  - "news"
  - "author:lxfater"
timestamp: "2026-05-01T16:18:55.563Z"
x_tweet_id: "2050240636020732015"
x_author: "铁锤人"
x_handle: "lxfater"
x_created_at: "2026-05-01T15:47:13.000Z"
x_engagement:
  likes: 0
  retweets: 0
  replies: 1
  bookmarks: 0
  views: 0
x_selected: 1
x_priority: 0
x_related_topic:

x_source: "crawler"
type_okf: "X Tweet"
source_url: "https://x.com/i/status/2050240636020732015"
created: "2026-05-01"
updated: "2026-06-26"
sha256: "7ea8855cf3f4acc01a3b59c627458863e1b4d9c27aad556b04185ab87c4d8d3d"
sources:
  - "/sources/news/2026-05-01/00__lxfater__apple-support-app-打包进-claude-md-苹果十亿美元-apple-intelligence-投资.md"
---

# Apple Support App 打包进 Claude.md：苹果十亿美元 Apple Intelligence 投资正在被第三方工具链静默替代

## 摘要

**苹果内部工程团队已实际采用 Claude Code 工作流，而非自研 Apple Intelligence。**

- Apple Support App 的发布包中被发现包含 Claude.md 文件，这是 Claude Code 的标准项目配置标识
- 苹果此前投入超 10 亿美元建设 Apple Intelligence 基础设施，但一线工程团队的实际工具选择已发生偏移
- 这是继多家大厂被曝使用第三方 AI 编码工具后，又一个「战略投资 vs 工程落地」脱节的信号

gakki 锐评：十亿美元的 AI 战略叙事，挡不住工程师用脚投票选择最好用的工具。苹果的尴尬在于——你可以不做 AI 模型，但你不能假装自家工程团队不用别人的。

## 原文

Apple Support App发布打包进了Claude.md 文件

看来苹果$1B 建 Apple Intelligence已经彻底报废，都用上Cluade code，哈哈哈


## 元信息

- **作者**: 铁锤人 ([@lxfater](https://x.com/lxfater))
- **发布时间**: 2026-05-01T15:47:13.000Z
- **抓取时间**: 2026-05-01T16:18:55.563Z
- **精选类别**: 📰 头条
- **优先级**: 0
- **互动**: ❤️ 0 · 🔄 0 · 💬 1 · 🔖 0 · 👁 0

## Citations

[1] [Tweet on X](https://x.com/i/status/2050240636020732015) — @lxfater (铁锤人)
[2] [Author page](/entities/authors/lxfater.md)
