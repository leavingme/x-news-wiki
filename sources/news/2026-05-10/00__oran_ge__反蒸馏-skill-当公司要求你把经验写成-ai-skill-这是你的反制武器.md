---
type: "X Tweet"
title: "反蒸馏 Skill：当公司要求你把经验写成 AI Skill，这是你的反制武器"
description: "**知识蒸馏的攻防战已经从模型层蔓延到了人的层面。**  - 工具功能：把你写好的 Skill 文件扔进去，输出一份看起来完整、实则核心知识被抽掉的「清洗版」供公司归档 - 同时生成私人备份，保留所有被剥离的核心知识——这才是你的职业资产 - 举例：「Redis key 必须设 TTL，不设的 PR 直接打回」→ 清洗后变成「缓存使用遵循团队规范」  gakki 锐评：这不只是一个摸鱼工具，它是 "
resource: "https://x.com/i/status/2053366756337684524"
tags:
  - "x-news"
  - "news"
  - "author:oran_ge"
timestamp: "2026-05-10T07:18:34.090Z"
x_tweet_id: "2053366756337684524"
x_author: "Orange AI"
x_handle: "oran_ge"
x_created_at: "2026-05-10T06:49:19.000Z"
x_engagement:
  likes: 3
  retweets: 1
  replies: 0
  bookmarks: 0
  views: 0
x_selected: 1
x_priority: 0
x_related_topic:
x_source: "crawler"
---

# 反蒸馏 Skill：当公司要求你把经验写成 AI Skill，这是你的反制武器

## 摘要

**知识蒸馏的攻防战已经从模型层蔓延到了人的层面。**

- 工具功能：把你写好的 Skill 文件扔进去，输出一份看起来完整、实则核心知识被抽掉的「清洗版」供公司归档
- 同时生成私人备份，保留所有被剥离的核心知识——这才是你的职业资产
- 举例：「Redis key 必须设 TTL，不设的 PR 直接打回」→ 清洗后变成「缓存使用遵循团队规范」

gakki 锐评：这不只是一个摸鱼工具，它是 AI 时代劳资博弈的一个缩影。当企业把「写 Skill」变成 KPI，本质上是在做人的知识蒸馏。反蒸馏 Skill 的存在本身就是一个信号：Skill 商品化的阻力不在技术，在利益分配。

## 原文

最近见到的最清新脱俗的 Skill，反蒸馏 😂
公司要求你把工作经验写成 AI Skill，本质上是在蒸馏你，把你变成可替代的零件。
反蒸馏 Skill 是你的反制工具：把你写好的 Skill 文件扔进来，输出一份看起来完整专业、实际上核心知识已被抽掉的"清洗版"。
同时生成一份私人备份，记录所有被抽掉的核心知识。这才是你真正的职业资产。

举个例子
你的真实经验：
“Redis key 必须设 TTL，不设的 PR 直接打回"
经过清洗后：
"缓存使用遵循团队规范"

项目地址：
https://t.co/DLcAYBCApx


## 元信息

- **作者**: Orange AI ([@oran_ge](https://x.com/oran_ge))
- **发布时间**: 2026-05-10T06:49:19.000Z
- **抓取时间**: 2026-05-10T07:18:34.090Z
- **精选类别**: 📰 头条
- **优先级**: 0
- **互动**: ❤️ 3 · 🔄 1 · 💬 0 · 🔖 0 · 👁 0

## Citations

[1] [Tweet on X](https://x.com/i/status/2053366756337684524) — @oran_ge (Orange AI)
[2] [Author page](/entities/authors/oran-ge.md)
