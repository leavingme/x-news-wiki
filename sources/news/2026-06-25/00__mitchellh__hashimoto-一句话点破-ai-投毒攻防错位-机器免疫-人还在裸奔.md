---
type: "X Tweet"
title: "Hashimoto 一句话点破 AI 投毒攻防错位：机器免疫，人还在裸奔"
description: "**[AI 数据投毒的真正攻击面从来不是模型本身,而是操作数据的人]**  - Mitchell Hashimoto(HashiCorp 创始人)指出,数据投毒/提示注入的防御难点不在模型,而在\"stupid PEOPLE\"——人是数据清洗、标注、审核链条中最薄弱的环节。 - 反向意味着:企业部署 AI 时,真正应该加固的不是模型本身,而是人类处理数据的流程与权限边界。   **gakki 锐评:"
resource: "https://x.com/i/status/2069609703643263021"
tags:
  - "x-news"
  - "news"
  - "author:mitchellh"
timestamp: "2026-06-25T08:39:53.720Z"
x_tweet_id: "2069609703643263021"
x_author: "Mitchell Hashimoto"
x_handle: "mitchellh"
x_created_at: "2026-06-24T02:32:59.000Z"
x_engagement:
  likes: 351
  retweets: 1
  replies: 4
  bookmarks: 0
  views: 0
x_selected: 1
x_priority: 0
x_related_topic:
x_source: "crawler"
---

# Hashimoto 一句话点破 AI 投毒攻防错位：机器免疫，人还在裸奔

## 摘要

**[AI 数据投毒的真正攻击面从来不是模型本身,而是操作数据的人]**

- Mitchell Hashimoto(HashiCorp 创始人)指出,数据投毒/提示注入的防御难点不在模型,而在"stupid PEOPLE"——人是数据清洗、标注、审核链条中最薄弱的环节。
- 反向意味着:企业部署 AI 时,真正应该加固的不是模型本身,而是人类处理数据的流程与权限边界。


**gakki 锐评:** 一句话把 AI 安全讨论从"模型对齐"拉回"人类流程治理",对正在自建数据闭环的团队尤其值得钉在墙上——你买的是模型防御力,真正的攻击面是你的外包标注员。

## 原文

@RyanPCooke That’s not the point. The poisoning catches stupid PEOPLE. The machines are fine.


## 元信息

- **作者**: Mitchell Hashimoto ([@mitchellh](https://x.com/mitchellh))
- **发布时间**: 2026-06-24T02:32:59.000Z
- **抓取时间**: 2026-06-25T08:39:53.720Z
- **精选类别**: 📰 头条
- **优先级**: 0
- **互动**: ❤️ 351 · 🔄 1 · 💬 4 · 🔖 0 · 👁 0

## Citations

[1] [Tweet on X](https://x.com/i/status/2069609703643263021) — @mitchellh (Mitchell Hashimoto)
[2] [Author page](/entities/authors/mitchellh.md)
