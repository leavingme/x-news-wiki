---
type: "source"
title: "56 键迷你硬件键盘曝光：WiFi + 蓝牙 + 红外 + 6 轴运动传感器，Agent 硬件入口的又一块拼图"
description: "**小互曝光一款高集成度迷你键盘硬件，接口密度暗示其定位远超「输入设备」。**  - 56 键迷你键盘集成 WiFi、蓝牙、红外、麦克风、喇叭、6 轴运动传感器、microSD 卡槽，背面带磁吸 + 乐高接口 - 这种传感器密度 + 无线协议栈的组合，更像是一个可穿戴 Agent 终端的原型，而非传统外设 - 1750mAh 电池 + 磁吸/乐高模块化设计，指向可组合的边缘计算场景  gakki "
resource: "https://x.com/i/status/2052671868382146762"
tags:
  - "x-news"
  - "news"
  - "author:xiaohu"
timestamp: "2026-05-08T10:19:22.454Z"
x_tweet_id: "2052671868382146762"
x_author: "小互"
x_handle: "xiaohu"
x_created_at: "2026-05-08T08:48:04.000Z"
x_engagement:
  likes: 1
  retweets: 0
  replies: 4
  bookmarks: 0
  views: 0
x_selected: 1
x_priority: 0
x_related_topic:

x_source: "crawler"
type_okf: "X Tweet"
source_url: "https://x.com/i/status/2052671868382146762"
created: "2026-05-08"
updated: "2026-06-26"
sha256: "b4fc62ac5b482014616de038c078113450afd268703544ce2c03124034d9d377"
sources:
  - "/sources/news/2026-05-08/00__xiaohu__56-键迷你硬件键盘曝光-wifi-蓝牙-红外-6-轴运动传感器-agent-硬件入口的又一块拼图.md"
---

# 56 键迷你硬件键盘曝光：WiFi + 蓝牙 + 红外 + 6 轴运动传感器，Agent 硬件入口的又一块拼图

## 摘要

**小互曝光一款高集成度迷你键盘硬件，接口密度暗示其定位远超「输入设备」。**

- 56 键迷你键盘集成 WiFi、蓝牙、红外、麦克风、喇叭、6 轴运动传感器、microSD 卡槽，背面带磁吸 + 乐高接口
- 这种传感器密度 + 无线协议栈的组合，更像是一个可穿戴 Agent 终端的原型，而非传统外设
- 1750mAh 电池 + 磁吸/乐高模块化设计，指向可组合的边缘计算场景

gakki 锐评：硬件参数本身不算惊艳，但「把一切传感器塞进一个键盘大小的壳子里」的产品直觉值得关注——当 Agent 需要感知物理世界时，这类低成本密集传感器节点可能是最务实的入口。

## 原文

颜值在线

接口也挺多

56 键迷你键盘（按起来跟手机键盘差不多大）
1750mAh 电池
麦克风、喇叭、耳机孔
WiFi、蓝牙、红外、microSD 卡槽
6 轴运动传感器
背面有磁吸 + 乐高接口 https://t.co/4dK56S1WkZ


## 元信息

- **作者**: 小互 ([@xiaohu](https://x.com/xiaohu))
- **发布时间**: 2026-05-08T08:48:04.000Z
- **抓取时间**: 2026-05-08T10:19:22.454Z
- **精选类别**: 📰 头条
- **优先级**: 0
- **互动**: ❤️ 1 · 🔄 0 · 💬 4 · 🔖 0 · 👁 0

## Citations

[1] [Tweet on X](https://x.com/i/status/2052671868382146762) — @xiaohu (小互)
[2] [Author page](/entities/authors/xiaohu.md)
