---
type: "source"
title: "向阳乔木开源多Agent辩论Skill：3-5个独立Sub-agent互批盲点，Codex主持给出比单推理更优结论"
description: "**一句话核心判断：** 多Agent竞争性博弈而非协作，是当前提升推理质量最高效的工程路径。  基于论文实现的 joeseesun/qiaomu-heavyskill，通过强制视角冲突暴露单推理无法自我纠正的认知盲区。  关键机制：完全独立的Sub-agent先各自思考，Codex主持交叉辩论，最后收敛结论——这比CoT多了「外部他者」这层结构性约束。  gakki 锐评：多Agent讨论不是什"
resource: "https://x.com/i/status/2054238438879302106"
tags:
  - "x-news"
  - "news"
  - "author:vista8"
timestamp: "2026-05-12T19:18:52.491Z"
x_tweet_id: "2054238438879302106"
x_author: "向阳乔木"
x_handle: "vista8"
x_created_at: "2026-05-12T16:33:04.000Z"
x_engagement:
  likes: 16
  retweets: 2
  replies: 1
  bookmarks: 0
  views: 0
x_selected: 1
x_priority: 0
x_related_topic:

x_source: "crawler"
type_okf: "X Tweet"
source_url: "https://x.com/i/status/2054238438879302106"
created: "2026-05-12"
updated: "2026-06-26"
sha256: "c8dc5abf97fd4185a890e72b94851cf6d3a9601522bfbc8c31da6b651492bec9"
sources:
  - "/sources/news/2026-05-12/00__vista8__向阳乔木开源多agent辩论skill-3-5个独立sub-agent互批盲点-codex主持给出比单推理更优结论.md"
---

# 向阳乔木开源多Agent辩论Skill：3-5个独立Sub-agent互批盲点，Codex主持给出比单推理更优结论

## 摘要

**一句话核心判断：** 多Agent竞争性博弈而非协作，是当前提升推理质量最高效的工程路径。

基于论文实现的 joeseesun/qiaomu-heavyskill，通过强制视角冲突暴露单推理无法自我纠正的认知盲区。

关键机制：完全独立的Sub-agent先各自思考，Codex主持交叉辩论，最后收敛结论——这比CoT多了「外部他者」这层结构性约束。

gakki 锐评：多Agent讨论不是什么新概念，但开源Skill化+Codex主持的组合把工程门槛降到了零。问题是：当所有人都用同一个Skill「辩论」，辩论本身就不再是差异化优势——Skill只是起点，执行Skill的追问深度才是真正的护城河。

## 原文

免费开源基于这篇论文写的Skill

让 3–5 个完全独立 Sub agent 独立思考同一个问题，再由 Codex 主持讨论。

找出每个视角盲点，最后给出一个比单推理更好的结论。

安装
npx skills add joeseesun/qiaomu-heavyskill

开源地址：
https://t.co/AcsYKFjuf2

Demo讨论：
https://t.co/nuXM1EFNOU


## 元信息

- **作者**: 向阳乔木 ([@vista8](https://x.com/vista8))
- **发布时间**: 2026-05-12T16:33:04.000Z
- **抓取时间**: 2026-05-12T19:18:52.491Z
- **精选类别**: 📰 头条
- **优先级**: 0
- **互动**: ❤️ 16 · 🔄 2 · 💬 1 · 🔖 0 · 👁 0

## Citations

[1] [Tweet on X](https://x.com/i/status/2054238438879302106) — @vista8 (向阳乔木)
[2] [Author page](/entities/authors/vista8.md)
